...
export async function POST(request: NextRequest, { params }: ArticleProps) {
    ...
    try {
        ...
        const { title, summary, content, image, category } = await request.json();
        ...
        console.log('Request body:', { title, summary, content, image, category });

        const query = `
                        INSERT INTO article (
                        ${articleId !== '-1' ? 'articleid, ' : ''}title, summary, content, image, category
                        ) VALUES (
                            ${articleId !== '-1' ? '$1, $2, $3, $4, $5, $6' : '$1, $2, $3, $4, $5'}
                        )
                        ${articleId !== '-1' ? `
        ...
        const paramsArray = articleId !== '-1'
            ? [articleId, title, summary, content, image, category]
